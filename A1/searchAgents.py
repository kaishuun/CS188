...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        #gets the startingGameState as a default gamestate
        self.gameState = startingGameState
        #returns the corners in a list
        self.getcorners = [(1,1),(1,top),(right,1),(right,top)]
    ...
    def getStartState(self):
        ...
        #Resetting corners visited
        visitedcorners = []

        #returns original location + corners visited
    ...
    def isGoalState(self, state):
        ...
        #checks if all the corners are visited
        check = 4 == len(state[1])
        return check
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            "*** YOUR CODE HERE ***"
            #gathers the position and the corners visited
            x,y = state[0]
            corners = state[1]
            ...
            if not self.walls[nextx][nexty]:
                ...
                if (nextState in self.corners) and not(nextState in corners):
                    #adds on the corner to the visited list if it's on the fringe
...
def cornersHeuristic(state, problem):
    ...
    #gathers current state information
    x,y = state[0]
    visited = state[1]
    remainder = []
    sum = 0

    #Gathers List of dots not visited
    for i in corners:
        if not i in visited:
    ...
    result = []
    for i in remainder:
        result = result + [mazeDistance((x,y),i,problem.gameState)]
    #returns the farthest distance away from the dot

...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #uses a counter to store new ghostBeliefs
        allPossible = util.Counter()
        #beliefs at jail position = 100% when ghost is caught
        if noisyDistance == None:
            allPossible[self.getJailPosition()] = 1
        else:
            #updating beliefs by using B = P(noisyDistance |Trust Dist) * prior belief
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        #newPostDist[p] = Pr( ghost is at position p at time t + 1 | ghost is at position oldPos at time t )
        #for newPos, prob in newPosDist.items():
        #Updates the belief for each position in the square with transition probability and prior beliefs
        for p in self.legalPositions:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        particles = 0
        #adds particles from the first to last available square
        #breaks out of loop after all particles are sampled
        self.particles = []
    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle two special cases:
          ...
          2) When all particles receive 0 weight, they should be recreated from
             ...
             weight for a belief distribution can be found by
              calling totalCount
             on a Counter object
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        #allPossible = updated partciel locations
        allPossible = util.Counter()
        belief = self.getBeliefDistribution()

        if noisyDistance == None:
            #all particles are placed in Jail if ghost is in Jail
            allPossible[self.getJailPosition()] = self.numParticles
        else:
            #particles not in jail
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    #new particle weights belief*emissionModel
                    allPossible[p] = belief[p]*emissionModel[trueDistance]

        #checks for all empty weights and resets board
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            #gets the position distribution of each particle and samples it
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #uses a counter to add a count for each particle in that position, then normalize
        belief = util.Counter()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #creates list of all possible permutations and shuffles it to get a uniform dist
        self.particles = []
        permutations = list(itertools.product(self.legalPositions,repeat = self.numGhosts))
        random.shuffle(permutations)
        #appends the suffled list to the particles domain until it hits the max amount
        particles = 0
        while particles < self.numParticles:
            for loc in permutations:
        ...
        for part in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if noisyDistances[i] == None:
                    ...
                    #ghost not in jail
                    #updates weightrs
                    weight = weight * emissionModels[i][util.manhattanDistance(part[i], pacmanPosition)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            #print(oldParticle)
            for i in range(self.numGhosts):
                #gets possition distributions for the ith ghost and samples it
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        belief = util.Counter()
        #sums up number of particles in each position and normalizes
        for part in self.particles:
                belief[part] += 1
